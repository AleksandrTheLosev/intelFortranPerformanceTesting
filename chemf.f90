MODULE chemf
  ...
  CONTAINS
    ...
    SUBROUTINE chemistry_step(Conc,time1,time2,O2_in,N2_in,M_in,H2O_in,TEMP_in,exp_coszen,Emi_iso, Emi_alp, CS)
      REAL(dp), INTENT(inout) :: Conc(neq)
      REAL(dp), INTENT(in)    :: time1, time2, O2_in, N2_in, M_in, H2O_in, TEMP_in
      REAL(dp), INTENT(in)    :: exp_coszen
      REAL(dp), INTENT(in) :: Emi_iso, Emi_alp
      REAL(DP), DIMENSION(2), intent(in) :: CS
      ...
      CALL calculate_k(exp_coszen,Emi_iso, Emi_alp, CS)
    ...
    SUBROUTINE calculate_k(exp_coszen, Emi_iso, Emi_alp, CS)
       ...
       k_rate(30) = CS(1)                                                                                             ! H2SO4 = H2SO4_P
       k_rate(31) = Emi_alp                                                                                           ! Emission rate of alpha-pinene
       k_rate(32) = Emi_iso                                                                                           ! Emission rate of isoprene
   ...
   SUBROUTINE f_lsode(neq, time, Conc, Conc_dot)
     ...
     REAL(dp), INTENT(out) :: Conc_dot(neq)
      ...
      ! 2 = O1D                O3conc              O1Dconc
      Conc_dot(2)  = k_rate(1)*Conc(1) - k_rate(2)*Conc(2)*H2O - k_rate(3)*Conc(2)*N2 - k_rate(4)*Conc(2)*O2

      ! 3 = OH
      Conc_dot(3)  = 2 * (k_rate(2)*Conc(2)*H2O) &    ! O1D + H2O = OH + OH
                    - k_rate(7)*Conc(3)*Conc(9)     &    ! OH + CO = HO2 + CO2 + REST
                    - k_rate(8)*Conc(3)*Conc(11)    &    ! OH + CH4 = CH3O2 + REST
                    - k_rate(9)*Conc(3)*Conc(13)   &    ! OH + MVK = HO2 + CH2O + REST
                    - k_rate(10)*Conc(3)*Conc(15)    &    ! HO2 + NO = OH + NO2
                    + k_rate(11)*Conc(8)*Conc(6)    &    ! isoprene + OH = RO2
                    - k_rate(14)*Conc(3)*Conc(7)    &    ! OH + CH2O = HO2 + REST
                    - k_rate(18)*Conc(3)*Conc(5)    &    ! OH + NO2 = HNO3
                    - k_rate(20)*Conc(3)*Conc(8)    &    ! OH + HO2 = H2O + O2
                    - k_rate(21)*Conc(3)*Conc(16)   &    ! OH + H2O2 = H2O + HO2
                    + k_rate(28)*Conc(8)*Conc(1)    &    ! HO2 + O3 = OH + O2 + O2
                    - k_rate(29)*Conc(3)*Conc(20)   &    ! SO2 + OH = H2SO4
                    - k_rate(33)*Conc(3)*Conc(23)        ! OH + Alpha-pinene = Rest
      ...
      Conc_dot(7) = + k_rate(10)*Conc(3)*Conc(15)  &     ! OH + MVK = HO2 + CH2O + REST
                    - k_rate(14)*Conc(3)*Conc(7)   &     ! OH + CH2O = HO2 + REST
                    - k_rate(6)*Conc(7)            &     ! CH2O = HO2 + REST
                    + k_rate(12)*Conc(12)*Conc(6)  &     ! CH3O2 + NO = HO2 + NO2 + CH2O + REST
                    + k_rate(13)*Conc(14)*Conc(6)        ! RO2 + NO = HO2 + NO2 + CH2O + MVK

      ! 8 = HO2
      Conc_dot(8) = + k_rate(6)*Conc(7)            &     ! CH2O = HO2 + REST
                    + k_rate(7)*Conc(3)*Conc(9)    &     ! OH + CO = HO2 + CO2 + REST
                    + k_rate(10)*Conc(3)*Conc(15)    &   ! OH + MVK = HO2 + CH2O + REST
                    - k_rate(11)*Conc(8)*Conc(6)   &     ! HO2 + NO = OH + NO2
                    + k_rate(12)*Conc(12)*Conc(6)  &     ! CH3O2 + NO = HO2 + NO2 + CH2O + REST
                    + k_rate(13)*Conc(14)*Conc(6)  &     ! RO2 + NO = HO2 + NO2 + CH2O + MVK
                    + k_rate(14)*Conc(3)*Conc(7)   &     ! OH + CH2O = HO2 + REST
                    -2*k_rate(15)*(Conc(8)**2) &             ! 2HO2 = H2O2 + O2
                    - k_rate(16)*Conc(12)*Conc(8)  &     ! CH3O2 + HO2 = REST
                    - k_rate(17)*Conc(14)*Conc(8)  &      ! RO2 + HO2 = REST
                    - k_rate(20)*Conc(3)*Conc(8)   &     ! OH + HO2 = H2O + O2
                    + k_rate(21)*Conc(3)*Conc(16)  &     ! OH + H2O2 = H2O + HO2
                    - k_rate(28)*Conc(8)*Conc(1)        ! HO2 + O3 = OH + O2 + O
      ! 9 = CO
      Conc_dot(9)  = 0.
      ...
      ! 12 = CH3O2
      Conc_dot(12) = + k_rate(8)*Conc(3)*Conc(11)   &    ! OH + CH4 = CH3O2 + REST
                     - k_rate(12)*Conc(12)*Conc(6)  &    ! CH3O2 + NO = HO2 + NO2 + CH2O + REST
                     - k_rate(16)*Conc(12)*Conc(8)       ! CH3O2 + HO2 = REST

      ! 13 = Isoprene
      Conc_dot(13) =  - k_rate(9)*Conc(13)*Conc(3) &     ! isoprene + OH = RO2
                      + k_rate(32)  &          ! Emission rate of isoprene
                      - k_rate(35)*Conc(1)*Conc(13)

      ! 14 = RO2
      Conc_dot(14) = + k_rate(9)*Conc(13)*Conc(3)   &    ! isoprene + OH = RO2
                     - k_rate(17)*Conc(14)*Conc(8)  &    ! RO2 + HO2 = REST
                     - k_rate(13)*Conc(14)*Conc(6)       ! RO2 + NO = HO2 + NO2 + CH2O + MVK

      ! 15 = MVK
      Conc_dot(15) = - k_rate(10)*Conc(3)*Conc(15)  &    ! OH + MVK = HO2 + CH2O + REST
                     + k_rate(13)*Conc(14)*Conc(6)       ! RO2 + NO = HO2 + NO2 + CH2O + MVK

      ! 16 = H2O2
      Conc_dot(16) = + k_rate(15)*(Conc(8)**2) &               ! 2HO2 = H2O2 + O2
                     - k_rate(21)*Conc(3)*Conc(16)    ! OH + H2O2 = H2O + HO2

      ! 17 = HNO3
      Conc_dot(17) = + k_rate(18)*Conc(3)*Conc(5)     &  ! OH + NO2 = HNO3
                     + 2 * k_rate(26)*Conc(19)*H2O &  ! N2O5 + H2O = HNO3 + HNO3
                     + k_rate(27)*Conc(19)*2*(H2O**2)  - k_rate(30) * Conc(17)  ! N2O5 + H2O + H2O = HNO3 + HNO3 + H2O

      ! 18 = NO3
      Conc_dot(18) = - k_rate(22)*Conc(6)*Conc(18)  &    ! NO + NO3 = NO2 + NO2
                     + k_rate(23)*Conc(5)*Conc(1)        &    ! NO2 + O3 = NO3 + O2
                     - k_rate(24)*Conc(5)*Conc(18)  &    ! NO2 + NO3 = N2O5
                     + k_rate(25)*Conc(19)               ! N2O5 = NO2 + NO3

      ! 19 = N2O5
      Conc_dot(19) = + k_rate(24)*Conc(5)*Conc(18)    &  ! NO2 + NO3 = N2O5
                     - k_rate(25)*Conc(19)            &  ! N2O5 = NO2 + NO3
                     - k_rate(26)*Conc(19)*H2O &  ! N2O5 + H2O = HNO3 + HNO3
                     - k_rate(27)*Conc(19)*(H2O**2)   ! N2O5 + H2O + H2O = HNO3 + HNO3 + H2O <-Check here

      ! 20 = SO2
      Conc_dot(20) = 0.

      ! 21 = H2SO4
      Conc_dot(21) = + k_rate(29)*Conc(20)*Conc(3) - k_rate(30)*Conc(21)      ! SO2 + OH = H2SO4

      !22 = H2SO4_P
      Conc_dot(22) = + k_rate(30)*Conc(21)               ! H2SO4 = H2SO4_P

      ! 23 = Alpha-pinene
      Conc_dot(23) = k_rate(31)  &              ! Emission rate of alpha-pinene
                    - k_rate(33)*Conc(3)*Conc(23)  &    ! OH + Alpha-pinene = Rest
                     - k_rate(34)*Conc(1)*Conc(23)     ! O3 + Alpha-pinene = Rest
                     

      !24 = HNO3_P
      !Conc_dot(24) = k_rate(7)*Conc(24) - k_rate(7)*Conc(3)*H2O - k_rate(3)*Conc(2)*N2 - k_rate(4)*Conc(2)*O2
      Conc_dot(24) = k_rate(30)*Conc(17)

      !25 = ELVOC
      Conc_dot(25) = 0.05*k_rate(33)*Conc(3)*Conc(23) + 0.10*k_rate(34)*Conc(1)*Conc(23) - k_rate(36)*Conc(25) ! OK
